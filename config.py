...
"""
微舆配置文件
注意：所有配置（包括数据库和 LLM）现在都可以通过网页界面进行配置！
配置会保存在浏览器本地存储（localStorage）中，也支持 WebDAV 云备份。
本文件仅作为备用配置或环境变量未设置时的默认值。
"""
...
# ============================== 数据库配置（推荐通过网页配置） ==============================
# ⚙️ 推荐使用网页配置界面进行配置！
# 点击网页右上角的"⚙️ 配置"按钮，在"数据库配置"部分填写以下信息。
# 配置会自动保存在浏览器本地，也可以通过 WebDAV 云备份。
#
# 如果你想在本地文件中配置，可以修改下面的默认值或设置环境变量。
...
# 
# 注意：数据库仅用于 Insight Engine 和 MindSpider 爬虫。
# 如果不使用这两个功能，可以不配置数据库。
